...
export class ChatGPTAdapter extends BaseAdapterPlugin {
  ...
  private pendingMcpAttachment: boolean = false;
  ...
  async submitForm(options?: { formElement?: HTMLFormElement }): Promise<boolean> {
    ...
    try {
      ...
      // Only MCP-injected file messages should trigger the post-submit reload.
      const shouldReloadAfterSubmit =
        this.pendingMcpAttachment && !!document.querySelector(this.selectors.FILE_PREVIEW);
      const previousUserTurnCount = shouldReloadAfterSubmit
      ...
      if (shouldReloadAfterSubmit) {
        this.waitForSubmittedFileThenReload(previousUserTurnCount);
      }
  ...
  async attachFile(file: File, options?: { inputElement?: HTMLInputElement }): Promise<boolean> {
    ...
    try {
      ...
      if (previewFound) {
        ...
        this.pendingMcpAttachment = true;
        this.context.logger.debug(`File attached successfully: ${file.name}; pending MCP attachment state enabled`);
        return true;
      } else {
  ...
  private waitForSubmittedFileThenReload(previousUserTurnCount: number): void {
    const guardKey = 'superpower-file-reload';
    const lastReload = Number(sessionStorage.getItem(guardKey) || '0');

    if (Date.now() - lastReload < 5000) {
      this.context.logger.warn('Skipping automatic reload because a recent MCP file reload was detected');
      return;
    }
    ...
    const observer = new MutationObserver(() => {
      ...
      const currentUserTurnCount = document.querySelectorAll('[data-message-author-role="user"]').length;

      if (currentUserTurnCount > previousUserTurnCount) {
        finished = true;
        observer.disconnect();
        this.pendingMcpAttachment = false;
        sessionStorage.setItem(guardKey, String(Date.now()));
        this.context.logger.debug('Submitted MCP file detected; reloading ChatGPT page');
    ...
    setTimeout(() => {
      if (!finished) {
        ...
        observer.disconnect();
        this.pendingMcpAttachment = false;
        this.context.logger.warn('Timed out waiting for submitted MCP file; automatic reload cancelled');
      }
    }, 15000);
  }
