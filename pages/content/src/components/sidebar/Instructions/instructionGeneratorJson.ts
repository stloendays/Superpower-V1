...
You can ask user to invoke one or more functions by writing a JSON Lines code block like the following as part of your reply to the user, YOU MAY INVOKE BETWEEN ONE AND FIVE FUNCTIONS IN A SINGLE RESPONSE. NEVER INVOKE MORE THAN FIVE FUNCTIONS IN ONE RESPONSE, It should be a JSON Lines code block like this:
...
5. You MAY include up to five independent function calls in one response when all required parameters are already known.\n5a. CRITICAL MULTI-CALL FORMAT: when emitting more than one function call in the same response, put EACH function call in its OWN separate fenced code block. Each block must contain exactly one complete function call with one unique call_id. NEVER place two function_call_start events or two invoke calls in the same fenced code block.
6. If a later function depends on the output of an earlier function, stop at that dependency and wait for the required function result before continuing.
7. NEVER invoke more than five functions in a single response.
