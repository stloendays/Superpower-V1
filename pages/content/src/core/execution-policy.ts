...
/**
 * Deterministic execution risk classifier. It does not inspect argument values and
 * therefore can be used in logs/telemetry without leaking prompt or credential data.
 *
 * `audit` mode is the compatibility default: risky calls are labelled but not blocked.
 * `guarded` mode reports a confirmation decision for high/critical side effects so a
 * UI/controller can gate automatic execution without changing MCP schemas.
 */
