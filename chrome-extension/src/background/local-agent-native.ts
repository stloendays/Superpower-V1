...
chrome.runtime.onMessage.addListener(
  (message: unknown, sender, sendResponse: (response: LocalAgentBridgeResponse) => void) => {
    ...
    chrome.runtime
      ...
      .catch(error => {
        ...
        sendResponse({
          success: false,
          error:
            detail || 'Superpower Local Agent is not available. Install the C++ native host and restart the browser.',
        });
      });
