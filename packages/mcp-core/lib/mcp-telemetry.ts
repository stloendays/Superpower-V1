import type { ToolRisk } from './execution-policy.js';
...
/** Session-local, payload-free MCP telemetry. */
export class McpTelemetry {
  private records: McpTelemetryRecord[] = [];

  constructor(private readonly maxRecords = 300) {}
  ...
  success(call: McpTelemetryPendingCall, result: unknown): McpTelemetryRecord {
    return this.complete(call, 'success', { resultChars: getSerializedLength(result) });
  }

  error(call: McpTelemetryPendingCall, error: unknown): McpTelemetryRecord {
    return this.complete(call, 'error', { errorKind: getErrorKind(error) });
  }
  ...
  private complete(
    ...
    if (this.records.length > Math.max(1, this.maxRecords)) this.records.shift();
    return record;
  }
